...
void obtenerHoraHumedad(char *linea, int *hora, float *humedad) {
    char marca = ';';
    int cont = 0;
    char *horario=(char*)malloc(sizeof(char)*2);
    char *token;
...
void calcularHumedadProm(FILE * binario) {
    ...
    for (int i = 0; i < 24; i++) {
        ...
        medicion.humProm = (medicion.humMax - medicion.humMin)/60;
...
void imprimirBinario(FILE * binario){
    ...
    for (int i = 0; i < 24; i++) {
        ...
        printf("hora: %d \n",medFinal.hora);
        printf("humedad maxima %4.2f \n",medFinal.humMax);
        printf("humedad min;%4.2f \n", medFinal.humMin );
        printf("humedad promen dicha hora: %4.2f \n", medFinal.humProm);
...
int main(int argc, char** argv) {
    int hora = 0;
    float humedad = 0;
    char * linea=(char*)malloc(sizeof(char)*LONG_FRASE);
    st_Medicion med;
    ...
    archivoTxt = fopen("mediciones.txt", "r+");
